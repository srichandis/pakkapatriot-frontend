...
import { ArrowLeft, Skull, Copy, Check, Info, X } from "lucide-react";

/**
 * Game server for online multiplayer.
 * - Local dev: run `npm run server` (server.js at the repo root, port 8377).
 * - Override for production by setting VITE_VAM_SERVER (e.g. wss://games.yourdomain.com).
 */
const DEFAULT_SERVER = (import.meta.env.VITE_VAM_SERVER as string | undefined) || "ws://localhost:8377";
...
export default function VishAmritGamePage() {
  ...
  const gameSrc = `/vishamrit/index.html?${qs.toString()}`;
  ...
  return (
    <div className="h-dvh w-full flex flex-col bg-[#0F1912]">
      {/* Slim control bar */}
      <div className="flex items-center justify-between gap-3 px-3 sm:px-5 py-2.5 bg-[#0F1912] border-b border-[#2A4A32] text-white shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="hidden sm:flex items-center gap-1.5 text-sm font-black tracking-wide min-w-0">
            <Skull size={15} className="text-[#a33a5e] shrink-0" />
            <span className="truncate">Vish &amp; Amrit</span>
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {serverReady !== null && (
            <span
              ...
              title={serverReady
                ...
                : `No game server at ${server} — online mode will be offline. Start it with "npm run server" or set VITE_VAM_SERVER.`}
            >
          ...
          <button
            onClick={copyLink}
            className="flex items-center gap-1.5 text-[11px] font-bold text-[#B5CADF] hover:text-white border border-[#2A4A32] hover:border-[#F6B828] px-2.5 py-1.5 rounded-full transition-colors"
      ...
      <div className="flex-1 min-h-0 relative bg-[#FCFAF5]">
        <iframe
          src={gameSrc}
          title="Vish &amp; Amrit — the poison chase, an ancient freeze-tag board game of Bhārat"
          className="absolute inset-0 w-full h-full border-0"
          allow="clipboard-write; autoplay"
        />
        {serverReady === false && !bannerDismissed && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 max-w-md w-[calc(100%-24px)] bg-[#0F1912]/95 backdrop-blur text-white text-[11px] font-medium px-4 py-2.5 rounded-xl border border-[#2A4A32] shadow-2xl flex items-start gap-2">
            ...
            <span className="flex-grow">
              ...
              <code className="text-[#F6B828]">npm run server</code> and set <code className="text-[#F6B828]">VITE_VAM_SERVER</code>.
            </span>
            <button
