...
import { ArrowLeft, LayoutGrid, Copy, Check, Info, X } from "lucide-react";

/**
 * Game server for online multiplayer.
 * - Local dev: run `npm run server` (server.js at the repo root, port 8377).
 * - Override for production by setting VITE_CKV_SERVER (e.g. wss://games.yourdomain.com).
 */
const DEFAULT_SERVER = (import.meta.env.VITE_CKV_SERVER as string | undefined) || "ws://localhost:8377";
...
export default function ChaturvimshatiGamePage() {
  ...
  const gameSrc = `/chaturvimshati/index.html?${qs.toString()}`;
  ...
  return (
    <div className="h-dvh w-full flex flex-col bg-[#1d1026]">
      {/* Slim control bar */}
      <div className="flex items-center justify-between gap-3 px-3 sm:px-5 py-2.5 bg-[#1d1026] border-b border-[#3a1f4a] text-white shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="hidden sm:flex items-center gap-1.5 text-sm font-black tracking-wide min-w-0">
            <LayoutGrid size={15} className="text-[#F6B828] shrink-0" />
            <span className="truncate">Chaturvimshati Koṣṭaka</span>
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {serverReady !== null && (
            <span
              ...
              title={serverReady
                ...
                : `No game server at ${server} — online mode will be offline. Start it with "npm run server" or set VITE_CKV_SERVER.`}
            >
          ...
          <button
            onClick={copyLink}
            className="flex items-center gap-1.5 text-[11px] font-bold text-[#B5CADF] hover:text-white border border-[#3a1f4a] hover:border-[#F6B828] px-2.5 py-1.5 rounded-full transition-colors"
      ...
      <div className="flex-1 min-h-0 relative bg-[#FCFAF5]">
        <iframe
          src={gameSrc}
          title="Chaturvimshati Koṣṭaka — Twenty-Four Squares, ancient board game of Bhārat"
          className="absolute inset-0 w-full h-full border-0"
          allow="clipboard-write; autoplay"
        />
        {serverReady === false && !bannerDismissed && (
          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 max-w-md w-[calc(100%-24px)] bg-[#1d1026]/95 backdrop-blur text-white text-[11px] font-medium px-4 py-2.5 rounded-xl border border-[#3a1f4a] shadow-2xl flex items-start gap-2">
            ...
            <span className="flex-grow">
              ...
              <code className="text-[#F6B828]">{server}</code>. Play hot-seat or vs the computer now, or start the server with{" "}
              <code className="text-[#F6B828]">npm run server</code> and set <code className="text-[#F6B828]">VITE_CKV_SERVER</code>.
            </span>
            <button
